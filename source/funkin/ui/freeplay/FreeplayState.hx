...
class FreeplayState extends MusicBeatSubState
{
  ...
  public static var switchToMonsterSong:String = '';
  ...
  {
    if (Save.instance.charactersSeen.contains("spooky"))
    {
        currentCharacterId = "spooky";
        rememberedCharacterId = "spooky";
    }
    else
    {
    ...
    if (currentCharacterId == "spooky")
    {
        ostName.text = "UNOFFICIAL SOUNDTRACK";
    }
  ...
  {
    MainMenuState.FreeplayCurSelectedSAVED = 1;
  ...
  override function update(elapsed:Float):Void
  {
    ...
    if (stupidPooper == false && currentCharacterId != "bf")
    {
    ...
    if (controls.FREEPLAY_CHAR_SELECT && !busy && currentCharacterId == "spooky")
    {
      tryOpenCharSelect();
    }

    if (controls.FREEPLAY_FAVORITE && !busy && currentCharacterId == "spooky")
    {
  ...
  function handleInputs(elapsed:Float):Void
  {
    ...
    if ((controls.UI_UP || controls.UI_DOWN) && currentCharacterId == "spooky")
    {
      if (spamming)
      {
        if (spamTimer >= 0.07)
    ...
    if (controls.UI_LEFT_P && currentCharacterId == "spooky")
    {
      if (dj != null) dj.resetAFKTimer();
    ...
    if (controls.UI_RIGHT_P && currentCharacterId == "spooky")
    {
      if (dj != null) dj.resetAFKTimer();
      if (currentCharacterId == "bf")
        changeDiff(1);
    ...
    if (controls.BACK && !busy && currentCharacterId == "spooky")
    {
      busy = true;
    ...
    if (accepted && currentCharacterId == "spooky")
    {
