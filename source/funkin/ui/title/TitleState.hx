...
class TitleState extends MusicBeatState
{
  ...
  function startIntro():Void
  {
    ...
    skidDance.shader = swagShader.shader;
    ...
    pumpDance.shader = swagShader.shader;
  ...
  override function update(elapsed:Float):Void
  {
    ...
    if (pressedEnter && !transitioning && skippedIntro)
    {
      ...
      FlxG.mouse.visible = false;
      Save.instance.addCharacterSeen('pico');
      ...
      new FlxTimer().start(2, function(tmr:FlxTimer) {
        ...
        // Assets.cache.clear(Paths.image('skidDanceTitle'));
        // Assets.cache.clear(Paths.image('pumpDanceTitle'));
  ...
  override function beatHit():Bool
  {
    ...
    if (!skippedIntro)
    {
      ...
      if (Conductor.instance.currentBeat > lastBeat)
      {
        ...
        for (i in lastBeat...Conductor.instance.currentBeat)
        {
          ...
          {
            case 1:
              createCoolText(['The', 'Spooky Mix Team']);
            case 3:
    ...
    if (skippedIntro)
    {
      ...
      /*
      if (skidDance != null && skidDance.animation != null)
      {
        if (danceLeft)
        {
            skidDance.animation.play('danceRight');
            pumpDance.animation.play('danceRight');
        }
        else
        {
            skidDance.animation.play('danceLeft');
            pumpDance.animation.play('danceLeft');
        }
      }
      */
    }
