...
class ResultState extends MusicBeatSubState
{
  ...
  override function update(elapsed:Float):Void
  {
    ...
    if (controls.PAUSE && canPress == true)
    {
      canPress = false;
      FreeplayState.switchToMonsterSong = '';

      if (params.songId.toLowerCase() == 'south' && !Save.instance.monsterTranitionsSeen.contains("monster"))
      {
          new FlxTimer().start(0.16, _ ->
          {
                FunkinSound.playOnce(Paths.sound('thunder_1'), 2);
          });
          FlxG.camera.flash(FlxColor.WHITE, 1);
          cameraEverything.alpha = 0;
          cameraBG.alpha = 0;
          cameraScroll.alpha = 0;
      ...
      else if (params.songId.toLowerCase() == 'eggnog' && !Save.instance.monsterTranitionsSeen.contains("winter-horrorland"))
      {
          new FlxTimer().start(0.16, _ ->
          {
      ...
      else if (params.songId.toLowerCase() == 'south' && !Save.instance.monsterTranitionsSeen.contains("monster"))
      {
         ...
         FreeplayState.switchToMonsterSong = 'monster';
      }
      else if (params.songId.toLowerCase() == 'eggnog' && !Save.instance.monsterTranitionsSeen.contains("winter-horrorland"))
      {
         targetState = new funkin.ui.freeplay.FreeplayState();
         ...
         PlayStatePlaylist.isStoryMode = false;
         FreeplayState.switchToMonsterSong = 'winter-horrorland';
      }
      else
      {
