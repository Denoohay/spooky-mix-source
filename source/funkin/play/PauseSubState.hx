...
class PauseSubState extends MusicBeatSubState
{
  ...
  /**
   * A text object which displays the current song's mod art creator.
   */
  var metadataModArt:FlxText;
  ...
  function startPauseMusic():Void
  {
    ...
    trace('what the? this is the song path Im gettin: ' + pauseMusicPath);

    // Start playing at a random point in the song.
    pauseMusic.play(false);
    pauseMusic.fadeIn(5, 0, 0.5);
  }
  ...
  function buildMetadata():Void
  {
    ...
    if (PlayState.instance?.currentChart != null)
    {
      metadataArtist.text = 'Composer: ${PlayState.instance.currentChart.songArtist}';
    }
    metadataArtist.scrollFactor.set(0, 0);
    metadata.add(metadataArtist);

    metadataModArt = new FlxText(20, metadataArtist.y + 32, FlxG.width - 40, 'Mod Artist: ${Constants.DEFAULT_MODART}');
    ...
    metadataCharter = new FlxText(20, metadataModArt.y + 32, FlxG.width - 40, 'Charter: ${Constants.DEFAULT_CHARTER}');
