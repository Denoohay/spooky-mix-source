...
class Song implements IPlayStateScriptedClass implements IRegistryEntry<SongMetadata>
{
   ...
   * The default value for the song's artist
   */
  public static final DEFAULT_MODART:String = 'Unknown';
  ...
  /**
   * The creator of the mod art in the song.
   */
  public var modArt(get, never):String;

  function get_modArt():String
  {
    if (_data != null) return _data?.modArt ?? DEFAULT_MODART;
    if (_metadata.size() > 0) return _metadata.get(Constants.DEFAULT_VARIATION)?.modArt ?? DEFAULT_MODART;
    return DEFAULT_MODART;
  }
  ...
  function populateDifficulties():Void
  {
    ...
    for (metadata in _metadata.values())
    {
      ...
      for (diffId in metadata.playData.difficulties)
      {
        ...
        difficulty.modArt = metadata.modArt ?? Constants.DEFAULT_MODART;
...
class SongDifficulty
{
  ...
  public var modArt:String = Constants.DEFAULT_MODART;
